...
with tf.Session(config=tfConfig) as sess:
	...
	for m in range(CADN):
		...
		VgtN = len(Vgt)
		# load prediction
		Vpred24 = scipy.io.loadmat("resultsss_{0}/{1}/{2}.mat".format(opt.group,opt.load,CAD))["pointcloud"][:,0]
	...
	scipy.io.savemat("resultsss_{0}/{1}/testerror.mat".format(opt.group,opt.load),{
		"pred2GT": pred2GT_all,
		"GT2pred": GT2pred_all
	})
